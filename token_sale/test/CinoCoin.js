...
contract('CinoCoin', function(accounts) {
  ...
  it('initializes the contrat with the correct values', function() {
    return CinoCoin.deployed().then(function(instance) {
      tokenInstance = instance;
      return tokenInstance.name();
    }).then(function(name) {
      assert.equal(name, 'CinoCoin', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol){
      assert.equal(symbol, 'CC', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'CinoCoin v1.0', 'has the correct standard');
    });
  })
  ...
  it('transfers token ownership', function() {
    return CinoCoin.deployed().then(function(instance) {
      tokenInstance = instance;
      //Test require statment first by transferring something larger than the sender's balanceOf
